    ...
    if df_pattern_filtered_min.empty:
        ...
        if st.button("Mostra partite filtrate (Pattern)"):
            with st.expander("Partite filtrate dall'Analisi Pattern"):
                ...
                try:
                    csv6 = df_pattern_filtered_min.to_csv(index=False).encode("utf-8")
                    st.download_button("Scarica CSV (Pattern - Sezione 6)", csv6, file_name="sezione6_partite_filtrate.csv", mime="text/csv")
                except Exception as _e:
                    st.info(f"CSV non disponibile: {_e}")
