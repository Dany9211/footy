...
def run_auto_backtest(df_source, stake=1.0):
    """
    Backtest automatico su 1X2, BTTS SI e Over/Under goal 0.5–6.5 (NO 1.0/2.0).
    Riconosce nomi colonna con underscore o con punto (es. Odd_over_2_5 / Odd_over_2.5).
    """
    import pandas as pd
    ...
    def tot_goals(row):
        try:
            gh = float(row["Gol_Home_FT"])
            ga = float(row["Gol_Away_FT"])
        except Exception:
            return None
        return gh + ga
    ...
    for ln in lines:
        ...
        col_over = pick_col(over_candidates)
        if col_over:
        ...
        col_under = pick_col(under_candidates)
        if col_under:
    ...
    for market, (odd_col, win_fn) in market_map.items():
        ...
        for strategy in strategies:
            ...
            wr = (wins / bets * 100.0) if bets > 0 else 0.0
...
# --- SEZIONE 7: Backtesting Strategie (Automatico, Goals O/U 0.5–6.5) ---
