...
try:
    ...
    # ===== Helper: parsing minuti (es. "45+2", "12") -> int =====
    ...
    # ===== Helper: costruisci lista minuti da colonne primo_gol_* ... ottavo_gol_* =====
    _ord = ["primo","secondo","terzo","quarto","quinto","sesto","settimo","ottavo"]
    def _goal_cols_present(_df, side):
        cols = []
        for o in _ord:
            name = f"{o}_gol_{side}"
            # supporta anche varianti con maiuscole
    ...
    if _df_base.empty:
        ...
        # Colonne minuti gol (da colonne 'primo_gol_home..ottavo' e '...away')
        home_cols = _goal_cols_present(_df_base, "home")
        away_cols = _goal_cols_present(_df_base, "away")
        if not home_cols and not away_cols:
            st.warning("Colonne dei minuti gol non trovate: servono 'primo_gol_home'..'ottavo_gol_home' e/o la versione _away.")
        else:
            # ===== UI Pattern =====
