...
if 'Anno' in df.columns:
    ...
    st.error("Colonna 'Anno' non trovata. Il filtro per anno non sarà disponibile.")
...
try:
    ...
    _ord = ["primo","secondo","terzo","quarto","quinto","sesto","settimo","ottavo"]
    def _goal_cols_present(_df, side):
        cols = []
        for o in _ord:
            name = f"{o}_gol_{side}"
    ...
    def _times_from_row(row, side_cols):
        mins = []
        for c in side_cols:
            val = row.get(c, None)
            if val is None:
                continue
            m = _parse_minute_token(val)
            if m is not None:
                mins.append(m)
        mins.sort()
        return mins

    _df_base = filtered_df.copy() if 'filtered_df' in globals() else df.copy()
    ...
    if _df_base.empty:
        ...
        home_cols = _goal_cols_present(_df_base, "home")
        away_cols = _goal_cols_present(_df_base, "away")
        if not home_cols and not away_cols:
            st.warning("Colonne minuti gol non trovate: attese 'primo_gol_home..ottavo_gol_home' e/o _away.")
        else:
            ...
            # FORZATO: dopo 1° gol solo 1-0 o 0-1
            score_after_fg = st.selectbox("Punteggio dopo 1° gol", ["1-0","0-1"], index=0, key="pat_score_after_fg")
            ...
            if enable_second:
                ...
                # FORZATO: dopo 2° gol scegli tra 1-1, 2-0, 0-2 (multi-selezione)
                score_after_sg_allowed = st.multiselect("Punteggio dopo 2° gol (consenti)", ["1-1","2-0","0-2"], default=["1-1","2-0","0-2"], key="pat_score_after_sg_allowed")
            else:
                score_after_sg_allowed = []
            ...
            _work = _df_base.copy()
            _work["_home_times"] = _work.apply(lambda r: _times_from_row(r, home_cols), axis=1) if home_cols else [[]]*len(_df_base)
            _work["_away_times"] = _work.apply(lambda r: _times_from_row(r, away_cols), axis=1) if away_cols else [[]]*len(_df_base)
            ...
            def _first_second(row):
                ...
                # punteggi dopo 1° e 2° gol
                if fg[1] is None:
                    fg_score = None
                else:
                    fg_score = "1-0" if fg[1]=="Home" else "0-1"
                if sg[1] is None:
                    sg_score = None
                else:
                    if fg[1]=="Home" and sg[1]=="Home":
                        sg_score = "2-0"
                    elif fg[1]=="Away" and sg[1]=="Away":
                        sg_score = "0-2"
                    else:
                        sg_score = "1-1"
                return pd.Series({
                    "_FG_min": fg[0], "_FG_team": fg[1], "_FG_score": fg_score,
                    "_SG_min": sg[0], "_SG_team": sg[1], "_SG_score": sg_score
                })
            ...
            # vincolo punteggio dopo 1° gol
            _mask &= (_work["_FG_score"] == score_after_fg)
            ...
            if enable_second:
                ...
                if score_after_sg_allowed:
                    _mask &= _work["_SG_score"].isin(score_after_sg_allowed)
...
with st.expander("Mostra i risultati del Backtest"):
    ...
    if filtered_df.empty:
        ...
        def esegui_backtest(df_to_analyze, market, strategy, stake):
            ...
            market_map = {
                ...
                "2 (Trasferta)": ("Odd__Away", lambda row: row["Gol_Away_FT"] > row["Gol_Home_FT"]),
            ...
            for col in required_cols:
                if col not in df_to_analyze.columns:
                    # Invece di fermare l'esecuzione, restituisci valori vuoti
        ...
        stake = st.number_input("Stake per scommessa", min_value=1.0, value=1.0, step=0.5, key="backtest_stake")
        
        # Mercati e strategie da testare
        markets_to_test = ["1 (Casa)", "X (Pareggio)", "2 (Trasferta)", "Over 2.5 FT", "BTTS SI FT"]
        strategies_to_test = ["Back", "Lay"]
        
        all_results = []
        for market in markets_to_test:
            for strategy in strategies_to_test:
                vincite, perdite, numero_scommesse, profit_loss, roi, win_rate, odd_minima = esegui_backtest(filtered_df, market, strategy, stake)
                ...
                all_results.append({
                    ...
                    "Odd Minima per profitto": f"{odd_minima:.2f}"
                })
