...
def run_auto_backtest(df_source, stake=1.0):
    """
    Esegue un backtest automatico su più mercati e strategie e ritorna un DataFrame riepilogativo.
    Include 1X2, BTTS SI, Over/Under da 0.5 a 6.5 (goal).
    """
    ...
    def tot_goals(r):
        return (r["Gol_Home_FT"] + r["Gol_Away_FT"])

    market_map = {
        "1 (Casa)": ("Odd_Home", lambda r: r["Gol_Home_FT"] > r["Gol_Away_FT"]),
        "X (Pareggio)": ("Odd_Draw", lambda r: r["Gol_Home_FT"] == r["Gol_Away_FT"]),
        "2 (Trasferta)": ("Odd__Away", lambda r: r["Gol_Away_FT"] > r["Gol_Home_FT"]),
        "BTTS SI FT": ("BTTS_SI", lambda r: (r["Gol_Home_FT"] > 0 and r["Gol_Away_FT"] > 0)),
    }

    lines = [0.5, 1.5, 2.5, 3.5, 4.5, 5.5, 6.5]
    for ln in lines:
        key_over = f"Over {ln} FT"
        col_over = f"Odd_over_{str(ln).replace('.', '_')}"
        market_map[key_over] = (col_over, lambda r, t=ln: tot_goals(r) > t)

        key_under = f"Under {ln} FT"
        col_under = f"Odd_under_{str(ln).replace('.', '_')}"
        market_map[key_under] = (col_under, lambda r, t=ln: tot_goals(r) < t)
    ...
    for market, (odd_col, win_fn) in market_map.items():
        ...
        for strategy in strategies:
            ...
            inv = bets * stake
            roi = (pl / inv * 100.0) if inv > 0 else 0.0
            wr = (vincite / bets * 100.0) if bets > 0 else 0.0
...
st.write("Esegue il backtest su 1X2, BTTS SI e Over/Under da 0.5 a 6.5, in una singola tabella, usando le partite correnti dei filtri.")
...
# --- SOVRASCRITTURA FUNZIONE: run_auto_backtest (robusta O/U 0.5–6.5) ---
def run_auto_backtest(df_source, stake=1.0):
    import pandas as pd

    req = ["Gol_Home_FT", "Gol_Away_FT"]
    for c in req:
        if c not in df_source.columns:
            return pd.DataFrame(columns=["Mercato","Strategia","Scommesse","Vincite","Perdite","P/L","ROI %","Win Rate %","Odd Minima per profitto"])
    ...
    col_home = pick_col(["Odd_Home", "odd_home"])
    col_draw = pick_col(["Odd_Draw", "odd_draw"])
    col_away = pick_col(["Odd_Away", "Odd__Away", "odd_away", "odd__away"])
    if col_home: market_map["1 (Casa)"] = (col_home, lambda r: r["Gol_Home_FT"] > r["Gol_Away_FT"])
    if col_draw: market_map["X (Pareggio)"] = (col_draw, lambda r: r["Gol_Home_FT"] == r["Gol_Away_FT"])
    if col_away: market_map["2 (Trasferta)"] = (col_away, lambda r: r["Gol_Away_FT"] > r["Gol_Home_FT"])

    col_btts_si = pick_col(["BTTS_SI", "BTTS_Sì", "btts_si"])
    if col_btts_si:
        market_map["BTTS SI FT"] = (col_btts_si, lambda r: (r["Gol_Home_FT"] > 0 and r["Gol_Away_FT"] > 0))
    ...
    for ln in lines:
        ...
        over_targets = [
            f"Odd_over_{token_und}", f"Odd_Over_{token_und}", f"odd_over_{token_und}",
            f"Odd_over_{token_dot}", f"Odd_Over_{token_dot}", f"odd_over_{token_dot}",
        ]
        col_over = pick_col(over_targets)
        if col_over:
            market_map[f"Over {ln} FT"] = (col_over, lambda r, t=ln: tot_goals(r) > t)

        under_targets = [
            f"Odd_under_{token_und}", f"Odd_Under_{token_und}", f"odd_under_{token_und}",
            f"Odd_under_{token_dot}", f"Odd_Under_{token_dot}", f"odd_under_{token_dot}",
        ]
        col_under = pick_col(under_targets)
        if col_under:
            market_map[f"Under {ln} FT"] = (col_under, lambda r, t=ln: tot_goals(r) < t)
