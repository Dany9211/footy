...
# --- FILTRI POSIZIONE IN CLASSIFICA ---
st.sidebar.header("Filtri Posizione Classifica")

def create_position_filter(column_name, label):
    """Crea un filtro slider per le posizioni in classifica."""
    if column_name in df.columns:
        # Assicura che la colonna sia numerica e rimuovi i valori mancanti
        positions = pd.to_numeric(df[column_name], errors='coerce').dropna()
        if not positions.empty:
            ...
            filters[column_name] = selected_range
        else:
             st.sidebar.warning(f"La colonna '{label}' non contiene dati numerici validi per il filtro.")
    else:
        st.sidebar.error(f"Colonna '{column_name}' non trovata. Il filtro non sarà disponibile.")

create_position_filter("Home_Pos_Tot", "Posizione Totale Squadra Casa")
create_position_filter("Away_Pos_Tot", "Posizione Totale Squadra Trasferta")
create_position_filter("Home_Pos_H", "Posizione Casa (Partite in Casa)")
create_position_filter("Away_Pos_A", "Posizione Trasferta (Partite in Trasferta)")
...
# --- Session State and Callback for Bet365 Odds ---
# Initialize session state keys for min/max odds if they don't exist
keys_to_init = [
    'min_Odd_Home', 'max_Odd_Home',
    'min_Odd__Away', 'max_Odd__Away'
]
for key in keys_to_init:
    if key not in st.session_state:
        st.session_state[key] = ""

def update_odds_range(source_quote_key):
    """Callback to update min/max odds based on a single quote input."""
    quote_str = st.session_state.get(source_quote_key, "").replace(",", ".")
    try:
        quote_val = float(quote_str)
        min_odd = quote_val - 0.25
        max_odd = quote_val + 0.25
        # Determine which min/max keys to update
        if 'home' in source_quote_key:
            st.session_state['min_Odd_Home'] = f"{min_odd:.2f}"
            st.session_state['max_Odd_Home'] = f"{max_odd:.2f}"
        elif 'away' in source_quote_key:
            st.session_state['min_Odd__Away'] = f"{min_odd:.2f}"
            st.session_state['max_Odd__Away'] = f"{max_odd:.2f}"
    except (ValueError, TypeError):
        # Clear if the input is empty or invalid
        if 'home' in source_quote_key:
            st.session_state['min_Odd_Home'] = ""
            st.session_state['max_Odd_Home'] = ""
        elif 'away' in source_quote_key:
            st.session_state['min_Odd__Away'] = ""
            st.session_state['max_Odd__Away'] = ""
...
def add_range_filter_text(col_name, label=None):
    if col_name in df.columns:
        ...
        if not numeric_col_series.isnull().all():
            ...
            # Definisci le chiavi per il session_state
            min_key = f"min_{col_name}"
            max_key = f"max_{col_name}"
            
            # I widget ora sono controllati da st.session_state
            st.sidebar.text_input(f"Min {label or col_name}", key=min_key)
            st.sidebar.text_input(f"Max {label or col_name}", key=max_key)
            
            # Leggi i valori dal session_state per applicare il filtro
            min_val_from_state = st.session_state.get(min_key, "")
            max_val_from_state = st.session_state.get(max_key, "")
            
            if min_val_from_state.strip() != "" and max_val_from_state.strip() != "":
                try:
                    # Converti a float e applica il filtro
                    filters[col_name] = (float(min_val_from_state.replace(",", ".")), float(max_val_from_state.replace(",", ".")))
                except ValueError:
                    st.sidebar.error(f"Valori non validi per {label or col_name}. Inserisci numeri.")
                    if col_name in filters:
                        del filters[col_name]
            else:
                # Rimuovi il filtro se i campi sono vuoti
                if col_name in filters:
                    del filters[col_name]
        else:
...
st.sidebar.text_input(
    "Quota Home BET365",
    key="bet365_home_quote",
    on_change=update_odds_range,
    args=('bet365_home_quote',)
)

st.sidebar.text_input(
    "Quota Away BET365",
    key="bet365_away_quote",
    on_change=update_odds_range,
    args=('bet365_away_quote',)
)
...
for col, val in filters.items():
    ...
    if col in ["Odd_Home", "Odd_Draw", "Odd__Away", "Odd_Over_0.5", "Odd_over_1.5", 
                "Odd_over_2.5", "Odd_Over_3.5", "Odd_Over_4.5", "Odd_Under_0.5", 
                "Odd_Under_1.5", "Odd_Under_2.5", "Odd_Under_3.5", "Odd_Under_4.5", 
                "BTTS_SI", "Giornata", "Home_Pos_Tot", "Away_Pos_Tot", "Home_Pos_H", "Away_Pos_A"]:
...
# Funzioni di controllo migliorate
def check_first_goal_enhanced(row, first_home_score, first_away_score, min_first, max_first):
    ...
    # Verifica primaria: Il pattern di gol e il minutaggio corrispondono?
    ...
    # Verifica di conferma: Il risultato HT è coerente?
    # Non è necessario che il risultato HT sia esattamente uguale, basta che non sia inferiore
    # dato che il primo gol può essere l'unico gol del primo tempo.
    home_ht = row.get("Gol_Home_HT", 0)
    away_ht = row.get("Gol_Away_HT", 0)
...
def check_second_goal_enhanced(row, second_home_score, second_away_score, min_second, max_second):
    ...
    # Verifica di conferma: Il risultato FT è coerente?
    home_ft = row.get("Gol_Home_FT", 0)
    away_ft = row.get("Gol_Away_FT", 0)

    # Non è necessario che il risultato FT sia esattamente uguale, basta che non sia inferiore
...
if st.button("Avvia Analisi Pattern Gol"):
    ...
    try:
        ...
        odd_away_min_patt = float(odd_away_min_patt) if odd_away_min_patt.strip() else 1.0 # CORREZIONE QUI
    ...
    if df_pattern_filtered_min.empty:
        ...
        # --- NUOVA SEZIONE: Visualizzazione partite trovate ---
        ...
        with st.expander("Mostra Statistiche HT (Pattern)"):
            ...
            st.subheader(f"Doppia Chance HT ({len(df_after_start_min)})")
            df_dc_ht = calcola_double_chance(df_after_start_min, 'ht')
            styled_df = df_dc_ht.style.background_gradient(cmap='RdYlGn', subset=['Percentuale %'])
            st.dataframe(styled_df)
        ...
        # Doppia Chance FT
        st.subheader(f"Doppia Chance FT ({len(df_after_start_min)})")
        df_dc_ft = calcola_double_chance(df_after_start_min, 'ft')
        styled_df = df_dc_ft.style.background_gradient(cmap='RdYlGn', subset=['Percentuale %'])
        st.dataframe(styled_df)
...
st.write("Esecuzione **automatica** dei backtest su tutti i mercati principali (1X2, BTTS SI, Over/Under 1.5–4.5) con strategia Back e Lay.")

if filtered_df.empty:
    st.warning("Il DataFrame filtrato è vuoto, non è possibile eseguire il backtest.")
else:
    # Funzione per eseguire il backtest su un singolo mercato/strategia
    def esegui_backtest(df_to_analyze, market, strategy, stake):
        ...
        market_map = {
            ...
            # Over FT
            "Over 1.5 FT": ("Odd_over_1.5", lambda row: (row["Gol_Home_FT"] + row["Gol_Away_FT"]) > 1.5),
            "Over 2.5 FT": ("Odd_over_2.5", lambda row: (row["Gol_Home_FT"] + row["Gol_Away_FT"]) > 2.5),
            "Over 3.5 FT": ("Odd_Over_3.5", lambda row: (row["Gol_Home_FT"] + row["Gol_Away_FT"]) > 3.5),
            "Over 4.5 FT": ("Odd_Over_4.5", lambda row: (row["Gol_Home_FT"] + row["Gol_Away_FT"]) > 4.5),
            # Under FT
            "Under 1.5 FT": ("Odd_Under_1.5", lambda row: (row["Gol_Home_FT"] + row["Gol_Away_FT"]) < 2),
            "Under 2.5 FT": ("Odd_Under_2.5", lambda row: (row["Gol_Home_FT"] + row["Gol_Away_FT"]) < 3),
            "Under 3.5 FT": ("Odd_Under_3.5", lambda row: (row["Gol_Home_FT"] + row["Gol_Away_FT"]) < 4),
            "Under 4.5 FT": ("Odd_Under_4.5", lambda row: (row["Gol_Home_FT"] + row["Gol_Away_FT"]) < 5),
        }
        ...
        for _, row in df_clean.iterrows():
            try:
                ...
                if odd and odd > 0:
                    is_winning = bool(win_condition(row))
                    if strategy == "Back":
                        if is_winning:
                            vincite += 1
        ...
        roi = (profit_loss / investimento_totale) * 100 if investimento_totale > 0 else 0.0
        win_rate = (vincite / numero_scommesse) * 100 if numero_scommesse > 0 else 0.0
        odd_minima = 100 / win_rate if win_rate > 0 else 0.0
    ...
    # Stake input (default 1.0) e lancio automatico
    stake = st.number_input("Stake per scommessa (applicato a tutti i backtest)", min_value=1.0, value=1.0, step=0.5, key="auto_bt_stake")
    ...
    for mkt in all_markets:
        for strat in strategies:
            res = esegui_backtest(filtered_df, mkt, strat, stake)
            if res is not None:
                results.append(res)

    if not results:
        st.info("Nessuna scommessa idonea trovata con i mercati disponibili.")
    else:
        import pandas as pd
