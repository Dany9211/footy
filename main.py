...
# Filtro Anno
# Se 'Anno' manca o è vuoto, prova a ricostruirlo al volo prima di popolare la selectbox
import re as _re_year
def _ensure_year_column(_df):
    if "Anno" not in _df.columns or _df["Anno"].dropna().empty:
        # tenta Data
        if "Data" in _df.columns:
            _df["Anno"] = pd.to_datetime(_df["Data"], errors="coerce").dt.year
        # tenta Year
        if ("Anno" not in _df.columns or _df["Anno"].dropna().empty) and "Year" in _df.columns:
            _df["Anno"] = pd.to_numeric(_df["Year"], errors="coerce")
        # tenta Season
        if ("Anno" not in _df.columns or _df["Anno"].dropna().empty) and "Season" in _df.columns:
            def _season2y(x):
                s = str(x)
                m = _re_year.search(r'(\d{2,4})\s*/\s*(\d{2,4})', s)
                if m:
                    y1 = int(m.group(1))
                    if y1 < 100: y1 += 2000
                    return y1
                m2 = _re_year.search(r'(20\d{2}|19\d{2})', s)
                return int(m2.group(1)) if m2 else None
            _df["Anno"] = pd.Series([_season2y(v) for v in _df["Season"]])
        # tenta altre date comuni
        if "Anno" not in _df.columns or _df["Anno"].dropna().empty:
            for cand in ["date","data","match_date","giorno","day"]:
                if cand in [c.lower() for c in _df.columns]:
                    # trova il vero nome colonna con questo lowercase
                    for c in _df.columns:
                        if c.lower() == cand:
                            _df["Anno"] = pd.to_datetime(_df[c], errors="coerce").dt.year
                            break
                    break
    if "Anno" in _df.columns:
        _df["Anno"] = pd.to_numeric(_df["Anno"], errors="coerce")
        return _df[~_df["Anno"].isna()].copy()
    return _df
...
if "Anno" in df.columns:
    ...
    if not df_anni_numeric.empty:
        all_unique_years = sorted(df_anni_numeric.astype(int).unique(), reverse=True)
        dynamic_range_options_labels = ["Anno Corrente"] + [f"Ultimi {n} anni" for n in range(2,11)]
        ...
        if selected_anno_display == "Tutti":
            ...
            if len(all_unique_years) >= num_years_back:
                ...
                years_to_filter = all_unique_years
            if years_to_filter:
                filters["Anno"] = (min(years_to_filter), max(years_to_filter))
            else:
                if "Anno" in filters:
                    del filters["Anno"]
        else:
            try:
                filters["Anno"] = int(selected_anno_display)
            except Exception:
                if "Anno" in filters:
                    del filters["Anno"]
    else:
        # Nessun anno valido, ma non blocchiamo: mostriamo solo 'Tutti'
        st.sidebar.selectbox("Seleziona Anno", ["Tutti"], index=0, key="anno_fallback")
        if "Anno" in filters:
            del filters["Anno"]
else:
    # Se proprio manca la colonna, mostriamo solo 'Tutti' e proseguiamo
