...
# --- SEZIONE 6: Filtri Quote HOME/AWAY per Campionato selezionato + CSV risultati (INPUT MANUALI) ---
try:
    import pandas as pd
    ...
    if _df_base is None or _df_base.empty:
        ...
        if _df_base.empty:
            ...
            # 6) Output risultati + CSV
            with st.expander("Mostra risultati (Sezione 6 - Home/Away)"):
                st.dataframe(res_df)
                try:
                    csv6 = res_df.to_csv(index=False).encode("utf-8")
                    st.download_button(
                        "Scarica CSV risultati (Sezione 6 - Home/Away)",
                        csv6,
                        file_name="sec6_home_away_campionato.csv",
                        mime="text/csv"
                    )
    ...
    st.warning(f"Impossibile applicare i filtri Home/Away della Sezione 6: {_e6_outer}")
