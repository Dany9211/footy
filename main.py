...
# --- SEZIONE 6: Visualizza righe usate (filtrate per Campionato selezionato) ---
try:
    import pandas as pd

    # Individua il DF della sezione 6 (pattern) o ricade su filtered_df se esiste
    _candidates6 = ["df_pattern_filtered_min", "df_pattern_filtered", "df_pattern", "pattern_df", "filtered_df"]
    _df6 = None
    for _name in _candidates6:
        if _name in globals():
    ...
    # Seleziona il nome colonna del campionato
    def _camp_col_name(_df):
        for cc in ["Campionato", "campionato", "League", "league", "Camp"]:
            if cc in _df.columns:
                return cc
        return None

    # Applica il campionato scelto nei filtri (se presente)
    if _df6 is not None:
        _df_to_show = _df6.copy()
        _camp_col = _camp_col_name(_df_to_show)
        try:
            _selected_league = None
            # 1) Se esiste il dict 'filters' con chiave 'Campionato'
            if 'filters' in globals() and isinstance(filters, dict) and 'Campionato' in filters:
                _selected_league = filters['Campionato']
            # 2) Altrimenti prova da session_state (chiavi comuni)
            if _selected_league is None:
                for key in ["selected_campionato", "campionato", "Campionato"]:
                    if key in st.session_state and st.session_state[key]:
                        _selected_league = st.session_state[key]
                        break

            if _camp_col and _selected_league not in (None, "Tutti", "All"):
                # gestisci lista/tuple o singolo valore
                if isinstance(_selected_league, (list, tuple, set)):
                    _df_to_show = _df_to_show[_df_to_show[_camp_col].isin(list(_selected_league))]
                else:
                    _df_to_show = _df_to_show[_df_to_show[_camp_col] == _selected_league]
        except Exception as _e_filter_league:
            st.info(f"Filtro Campionato non applicato (info: {_e_filter_league})")

        with st.expander("Mostra partite filtrate (Sezione 6 - per Campionato selezionato)"):
            if _df_to_show is None or _df_to_show.empty:
                st.info("Nessuna riga per il campionato selezionato con i filtri correnti.")
            else:
                st.dataframe(_df_to_show)
                try:
                    csv6 = _df_to_show.to_csv(index=False).encode("utf-8")
                    st.download_button(
                        "Scarica CSV (Sezione 6 - campionato selezionato)",
                        csv6,
                        file_name="sezione6_partite_filtrate_campionato.csv",
                        mime="text/csv"
                    )
                except Exception as _e6_dl:
                    st.info(f"CSV non disponibile: {_e6_dl}")
    else:
        st.info("Nessun dataset filtrato trovato per la Sezione 6.")
except Exception as _e6_outer:
    st.warning(f"Impossibile mostrare le righe della Sezione 6: {_e6_outer}")
...
# --- SEZIONE 6: Mostra partite (vincolate al Campionato selezionato a sinistra) ---
try:
    import pandas as pd

    # Base DF: usa filtered_df se esiste (già filtrato dai filtri a sinistra), altrimenti df
    _base_df = None
    if 'filtered_df' in globals() and isinstance(filtered_df, pd.DataFrame) and not filtered_df.empty:
        _base_df = filtered_df.copy()
    elif 'df' in globals() and isinstance(df, pd.DataFrame) and not df.empty:
        _base_df = df.copy()

    # DF della Sezione 6 (pattern). Se assente, ricadiamo sul base_df
    _candidates6 = ["df_pattern_filtered_min", "df_pattern_filtered", "df_pattern", "pattern_df"]
    _df6 = None
    for _name in _candidates6:
        if _name in globals():
            _obj = globals().get(_name)
            if isinstance(_obj, pd.DataFrame) and not _obj.empty:
                ...
                break
    if _df6 is None:
        _df6 = _base_df.copy() if _base_df is not None else None

    # Rileva colonna campionato
    def _camp_col_name(_df):
        for cc in ["Campionato", "campionato", "League", "league", "Camp"]:
            if _df is not None and cc in _df.columns:
                return cc
        return None

    # Rileva il campionato selezionato nei filtri
    _selected_league = None
    try:
        if 'filters' in globals() and isinstance(filters, dict) and 'Campionato' in filters:
            if isinstance(filters['Campionato'], (list, tuple, set)):
                _list = list(filters['Campionato'])
                if _list:
                    _selected_league = _list[0]
            else:
                _selected_league = filters['Campionato']
    except Exception:
        pass

    # Se ancora None, prova a dedurlo da filtered_df (se è single-league)
    if _selected_league in (None, "Tutti", "All") and _base_df is not None:
        _cname = _camp_col_name(_base_df)
        if _cname is not None:
            _uniq = _base_df[_cname].dropna().unique().tolist()
            if len(_uniq) == 1:
                _selected_league = _uniq[0]

    # Applica il filtro campionato allo specifico DF della sezione 6
    if _df6 is not None:
        _cname6 = _camp_col_name(_df6)
        if _cname6 is not None and _selected_league not in (None, "Tutti", "All"):
            _df6 = _df6[_df6[_cname6] == _selected_league]

        with st.expander("Mostra partite filtrate (Sezione 6 - SOLO campionato selezionato)"):
            if _df6 is None or _df6.empty:
                st.info("Nessuna riga per il campionato selezionato con i filtri correnti.")
            else:
                st.dataframe(_df6)
        ...
        st.info("Dati non disponibili per la Sezione 6.")
except Exception as _e6_outer:
    st.warning(f"Impossibile mostrare le righe della Sezione 6: {_e6_outer}")
