...
# --- SEZIONE 6: Filtri Quote HOME/AWAY per Campionato selezionato + RISULTATI GOL (INPUT MANUALI) ---
try:
    import pandas as pd
    ...
    # 1) Dataset base: già filtrato dalla sidebar, se disponibile
    _df_base = None
    ...
    if _df_base is None or _df_base.empty:
        st.info("Nessun dato disponibile per i filtri della Sezione 6.")
    else:
        # 2) Individua il campionato selezionato in sidebar
        ...
        if _df_base.empty:
            st.info("Nessuna riga nel campionato selezionato con i filtri attuali.")
        else:
            st.markdown("**Filtri Sezione 6 (Quote Home/Away sul campionato selezionato)**")

            # 4) Rileva nomi colonne odds Home/Away in modo robusto
            def _norm(s: str) -> str:
                return s.lower().strip()

            norm_map = { _norm(c): c for c in _df_base.columns }

            home_col = None
            for cand in ["odd_home", "odds_home", "home_odd", "quota_home", "odd casa", "oddcasa", "odd1", "odd_1", "1"]:
                if cand in norm_map:
                    home_col = norm_map[cand]; break
            if home_col is None and "Odd_Home" in _df_base.columns:
                home_col = "Odd_Home"

            away_col = None
            for cand in ["odd_away", "odds_away", "away_odd", "quota_away", "odd trasferta", "oddtrasferta", "odd2", "odd_2", "2"]:
                if cand in norm_map:
                    away_col = norm_map[cand]; break
            if away_col is None:
                if "Odd_Away" in _df_base.columns:
                    away_col = "Odd_Away"
                elif "Odd__Away" in _df_base.columns:
                    away_col = "Odd__Away"

            res_df = _df_base.copy()

            # 5) INPUT MANUALI: Home min/max, Away min/max (con default calcolati)
            if home_col is not None:
                odds_home = pd.to_numeric(res_df[home_col], errors="coerce").dropna()
                if not odds_home.empty:
                    h_min_default, h_max_default = float(odds_home.min()), float(odds_home.max())
                else:
                    h_min_default, h_max_default = 1.01, 100.0
                col1, col2 = st.columns(2)
                with col1:
                    h_min = st.number_input("Quota Home Min", min_value=0.0, value=round(h_min_default, 2), step=0.01, key="sec6_home_min")
                with col2:
                    h_max = st.number_input("Quota Home Max", min_value=0.0, value=round(h_max_default, 2), step=0.01, key="sec6_home_max")
                if h_max < h_min:
                    h_max = h_min
                res_df[home_col] = pd.to_numeric(res_df[home_col], errors="coerce")
                res_df = res_df.dropna(subset=[home_col])
                res_df = res_df[(res_df[home_col] >= h_min) & (res_df[home_col] <= h_max)]
            else:
                st.info("Colonna quota Home non trovata nel dataset.")

            if away_col is not None and not res_df.empty:
                odds_away = pd.to_numeric(res_df[away_col], errors="coerce").dropna()
                if not odds_away.empty:
                    a_min_default, a_max_default = float(odds_away.min()), float(odds_away.max())
                else:
                    a_min_default, a_max_default = 1.01, 100.0
                col3, col4 = st.columns(2)
                with col3:
                    a_min = st.number_input("Quota Away Min", min_value=0.0, value=round(a_min_default, 2), step=0.01, key="sec6_away_min")
                with col4:
                    a_max = st.number_input("Quota Away Max", min_value=0.0, value=round(a_max_default, 2), step=0.01, key="sec6_away_max")
                if a_max < a_min:
                    a_max = a_min
                res_df[away_col] = pd.to_numeric(res_df[away_col], errors="coerce")
                res_df = res_df.dropna(subset=[away_col])
                res_df = res_df[(res_df[away_col] >= a_min) & (res_df[away_col] <= a_max)]
            else:
                st.info("Colonna quota Away non trovata nel dataset.")

            # 6) CALCOLO GOL E RISULTATI
            if not res_df.empty:
                goal_h_col = "Gol_Home_FT" if "Gol_Home_FT" in res_df.columns else None
                goal_a_col = "Gol_Away_FT" if "Gol_Away_FT" in res_df.columns else None

                if goal_h_col and goal_a_col:
                    # Crea colonne risultati
                    res_df["Gol_Totali"] = pd.to_numeric(res_df[goal_h_col], errors="coerce") + pd.to_numeric(res_df[goal_a_col], errors="coerce")
                    res_df["Esito_1X2"] = res_df.apply(lambda r: ("1" if r[goal_h_col] > r[goal_a_col] else ("2" if r[goal_a_col] > r[goal_h_col] else "X")), axis=1)
                    res_df["BTTS_SI"] = res_df.apply(lambda r: int((pd.to_numeric(r[goal_h_col], errors="coerce") > 0) and (pd.to_numeric(r[goal_a_col], errors="coerce") > 0)), axis=1)

                    # Over/Under (solo mezze linee)
                    lines = [0.5,1.5,2.5,3.5,4.5,5.5,6.5]
                    for ln in lines:
                        res_df[f"Over_{str(ln).replace('.','_')}"] = (res_df["Gol_Totali"] > ln).astype(int)
                        res_df[f"Under_{str(ln).replace('.','_')}"] = (res_df["Gol_Totali"] < ln).astype(int)
                    ...
                    # 1X2
                    for es in ["1","X","2"]:
                        summary_rows.append({"Metrica": f"Esito {es}", "Valore": res_df["Esito_1X2"].eq(es).sum(), "Percentuale %": pct(res_df["Esito_1X2"].eq(es).sum())})
                    # BTTS
                    summary_rows.append({"Metrica": "BTTS SI", "Valore": res_df["BTTS_SI"].sum(), "Percentuale %": pct(res_df["BTTS_SI"].sum())})
                    # O/U
                    for ln in lines:
                        keyo=f"Over_{str(ln).replace('.','_')}"; keyu=f"Under_{str(ln).replace('.','_')}"
                        summary_rows.append({"Metrica": f"Over {ln}", "Valore": int(res_df[keyo].sum()), "Percentuale %": pct(int(res_df[keyo].sum()))})
                        summary_rows.append({"Metrica": f"Under {ln}", "Valore": int(res_df[keyu].sum()), "Percentuale %": pct(int(res_df[keyu].sum()))})

                    import pandas as _pd
                    df_summary = _pd.DataFrame(summary_rows)

                    with st.expander("Riepilogo risultati (gol/esiti) – Sezione 6"):
                        st.dataframe(df_summary)

                else:
                    st.info("Colonne gol FT non trovate (richieste: 'Gol_Home_FT' e 'Gol_Away_FT').")

            # 7) Output dettagli + CSV
            with st.expander("Mostra risultati filtrati (Sezione 6 - Home/Away + Gol)"):
                st.dataframe(res_df)
                try:
                    csv6 = res_df.to_csv(index=False).encode("utf-8")
                    st.download_button(
                        "Scarica CSV risultati (Sezione 6 - Home/Away + Gol)",
                        csv6,
                        file_name="sec6_home_away_gol_campionato.csv",
                        mime="text/csv"
                    )
    ...
    st.warning(f"Impossibile applicare i filtri Home/Away della Sezione 6 con calcolo gol: {_e6_outer}")
